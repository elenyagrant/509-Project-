function [X, Y] = kernel_time_series_preproc(x, y, window_size, kernel_param, ridge_param)
...
A = zeros(n_days, n_pts+1); % i^th row is the coefficients for the spatial model at i^th day.

K = zeros(n_pts);
for i=1:n_pts
    for j=1:n_pts
        K(i,j) = exp(-kernel_param*norm(x(i,:) - x(j,:))^2);
    end
end
K = [ones(n_pts,1) K];

A = (K'*K + ridge_param*eye(n_pts+1)) \ (K'*y);
